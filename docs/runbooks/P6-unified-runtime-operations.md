...
先选择本机可用的 Compose 调用：Compose v2 插件可用时执行 `export COMPOSE='docker compose'`；仅安装旧式独立命令时执行 `export COMPOSE='docker-compose'`。两者使用完全相同的 `compose.yaml` 和参数。下文统一写作 `$COMPOSE`。
...
1. 停止 Compose：`$COMPOSE down --remove-orphans`。
...
验证使用临时 Compose 项目名和临时命名卷。记录启动、重启、health/session 结果后，执行 `$COMPOSE -p <temporary-project> down --volumes --remove-orphans`。不得复用现有持久卷或清理不属于该项目的卷。
