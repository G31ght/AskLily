...
- 基线变更：ADR-0001 已由项目负责人批准；仓库将从私有改为公开以获取可验证的 `main` 分支保护。
- 验收状态：**黄色 - 条件验收**
...
在公开和远端保护 API 复验完成前，不能给出绿色结论。原“私有仓库”P0 基线已由 ADR-0001 替换；公开仅适用于当前不含业务实现、真实数据或密钥的 P0 内容，并不放宽后续敏感信息治理。
...
- 项目负责人已批准 ADR-0001，授权将仓库公开以启用并验证 `main` 分支保护。
...
| GitHub 可见性变更 | 待复验 | ADR-0001 已批准；将由 Project Lead 改为公开并通过 API 复验。 |
...
- **执行中：** ADR-0001 批准后，将公开仓库以消除私有仓库分支保护能力阻塞；公开带来的敏感信息风险按 ADR 的提交前扫描和后续 ADR 复审控制。
...
ADR-0001 已获批准，Project Lead 将公开仓库、启用并复验 `main` 保护。远端门禁通过后，草稿 PR #1 仍须由项目负责人按 P0 阶段报告确认是否合并；P1 授权另行请求。
