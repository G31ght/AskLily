...
原“私有仓库”P0 基线已由 ADR-0001 替换。公开仅适用于当前不含业务实现、真实数据或密钥的 P0 内容，并不放宽后续敏感信息治理。`main` 已由 GitHub 实际强制 PR、1 项审批、`L0 governance check`、管理员约束、禁止强推和禁止删除。项目负责人已于 2026-07-20 接受 P0；但是 PR 作者与唯一可用 GitHub 审批账号相同，GitHub 拒绝自我审批，候选尚未合并，因此本阶段维持黄色。
...
- **独立审批阻塞：** 项目负责人已接受 P0，PR #1 已转为可审阅；但 GitHub 禁止作者使用同一账号审批自己的 PR。不得降低审批门槛或绕过保护直接合并。
- **首期 CI 限制：** P0 仅有 L0 治理门禁；不虚构 Python/TypeScript lint、类型检查、服务集成、E2E 或真实 Connector 验证。

## 建议与下一步

项目负责人已接受 P0，远端保护与 CI 已复验通过。请指定一名具有仓库写权限、且非 PR 作者的审批者完成 PR #1 审批；获得该审批后即可按保护规则合并。P1 授权另行请求。
