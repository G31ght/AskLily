...
- 远端 PR：[#1](https://github.com/G31ght/AskLily/pull/1)（已转为可审阅）
- 基线变更：ADR-0001 已由项目负责人批准并执行；仓库为公开可审查仓库，`main` 已启用远端保护。
- 验收状态：**绿色 - 可验收**

## 阶段结论

P0 的本地可控施工环境已经形成：Git 初始基线、工程目录骨架、忽略与协作治理、任务/ADR/Capability/完成包模板，以及不访问网络或数据源的 L0 CI 均已可重复验证。独立 Test Agent 已给出复验报告，未发现候选树内缺陷。公开 GitHub 单仓库、初始 `main` 和 P0 候选已推送，PR #1 的远端 `L0 governance check` 已通过。

原“私有仓库”P0 基线已由 ADR-0001 替换。公开仅适用于当前不含业务实现、真实数据或密钥的 P0 内容，并不放宽后续敏感信息治理。ADR-0002 已将单账号无法满足的独立审批数调整为 0；`main` 仍由 GitHub 实际强制 PR、`L0 governance check`、管理员约束、禁止强推和禁止删除。项目负责人已于 2026-07-20 接受 P0，独立 Test Agent 验收与最新远端 CI 均通过，满足 P0 绿色验收条件。
...
- 创建仓库 `G31ght/AskLily`，设置 `origin`，推送初始 `main` 与 P0 候选；ADR-0001 后仓库已公开，并创建 PR #1。
...
- 项目负责人已批准 ADR-0002；`main` 的独立审批数已复验为 0，其余远端保护项保持启用，PR #1 已复验为可合并。
...
| GitHub `main` 保护 | 通过 | GitHub API 已确认 PR、审批数 0、`L0 governance check`、管理员强制、禁止强推和禁止删除。 |

## 风险与限制

- **公开范围限制：** 仓库已公开；后续不得提交真实数据、密钥、私有基础设施信息或任何未按 ADR-0001 复审的敏感内容。
- **单维护者限制：** 独立 GitHub 审批数为 0；项目负责人明确接受、独立 Test Agent 验收和通过的 CI 共同构成当前人工与自动验收。新增任何写权限维护者后必须按 ADR-0002 恢复至少 1 项独立审批。
- **首期 CI 限制：** P0 仅有 L0 治理门禁；不虚构 Python/TypeScript lint、类型检查、服务集成、E2E 或真实 Connector 验证。

## 建议与下一步

项目负责人已接受 P0，远端保护与 CI 已复验通过，PR #1 已满足当前保护规则并可合并。Project Lead 将按受保护 PR 路径合并 P0 候选；P1 授权另行请求。
...
- ADR-0002：`docs/adr/0002-single-maintainer-merge-approval.md`
- 公开远端：`https://github.com/G31ght/AskLily`
- 可审阅 PR：`https://github.com/G31ght/AskLily/pull/1`
...
- ADR-0002 复验：审批数为 0，其余 `main` 保护项未变；PR #1 的 `mergeStateStatus` 为 `CLEAN`。
