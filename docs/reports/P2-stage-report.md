...
独立复检与远端 CI 通过后，建议项目负责人验收为“Demo 绿色”。P3 仅可按单独授权接入 Zabbix 的真实只读 Connector；不得把 P2 结果表述为真实系统或 Production 已验证。
