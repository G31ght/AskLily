...
export function AdminApp() {
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-main">
      <header className="admin-header"><div><p className="admin-kicker">ASKLILY · LOCAL CONTROL PLANE</p><h1>{NAVIGATION.find((item) => item.id === panel)?.label}</h1><p>仅管理已注册的本地能力；严格只读边界保持生效。</p></div><div className="admin-profile"><span className="status-dot"/> {session.runtime.declared_environment} · {session.runtime.data_sources.map((item) => item.data_level).join(" / ")}</div></header>
...
function System({ system }: { system: AdminSystem | null }) { return <section className="admin-grid system-grid"><article className="admin-card"><header><h2>运行边界</h2><span>只读</span></header><dl><dt>客户声明环境</dt><dd>{system?.runtime.declared_environment ?? "-"}</dd><dt>数据源</dt><dd>{system?.runtime.data_sources.map((item) => `${item.source_id}: ${item.connection_state}`).join("；") || "-"}</dd><dt>业务配置 Schema</dt><dd>{system?.configuration_schema ?? "-"}</dd><dt>写操作</dt><dd>{system?.read_only ? "已禁止" : "-"}</dd></dl></article><article className="admin-card"><header><h2>监控来源预检</h2><span>零网络</span></header>{system?.monitoring_source_readiness.length ? <ul>{system.monitoring_source_readiness.map((item) => <li key={item.source_id}>{item.source_kind}: {item.status} · {item.blockers.join(" / ")}</li>)}</ul> : <p className="admin-muted">当前仅有 Fixture；未声明 Zabbix 或 Prometheus。</p>}<p className="admin-muted">不读取 endpoint、凭据、PromQL 或原始监控数据。</p></article><article className="admin-card"><header><h2>当前限制</h2><span>不可由后台解除</span></header><ul>{system?.limitations.map((item) => <li key={item}>{item}</li>) ?? <li>正在读取系统状态</li>}</ul></article></section>; }
